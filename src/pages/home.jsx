...
export default class Home extends Page {
  ...
  setAnimationPos() {
    ...
    if (this.webgl) {
      this.webgl.forEach((w, i) => {
        ...
        const y = (STORE.viewport.height / 2) - (w.scale.y / 2) - (bounds.top / STORE.screen.height) * STORE.viewport.height
  ...
  updateDisplay() {
    if (this.displayMesh) {
      ...
      const x = -(STORE.viewport.width / 2) + (width / 2) + (bounds.left / STORE.screen.width) * STORE.viewport.width
      const y = (STORE.viewport.height / 2) - (height / 2) - (bounds.top / STORE.screen.height) * STORE.viewport.height
  ...
  addListeners() {
    this.media.forEach((m, i) => {
      m.addEventListener("click", () => {
        this.displayMesh.mesh.material = this.webgl[ i ].material
      })
    })
  }
  ...
  out() {
    return new Promise(resolve => {
      this.animaDisplay = this.displayMesh.mesh.material?.uniforms?.opacity
      ...
      piezo.timeline({
        ...
        targets: this.animaArray,
        delay: piezo.stagger(80, { from: 'last' }),
        y: -STORE.viewport.height,
      }).add({
        targets: this.animaDisplay,
        duration: 300,
        value: 0
      }, 100)
    })
